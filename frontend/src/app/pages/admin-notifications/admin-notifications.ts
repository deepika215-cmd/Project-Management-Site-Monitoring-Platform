...
export class AdminNotifications implements OnInit, OnDestroy {
  ...
  load(): void {
    ...
    this.api.getAllNotifications().pipe(timeout({ first: 10000 }), catchError(() => this.api.getMyNotifications().pipe(timeout({ first: 8000 })))).pipe(
  ...
  generate(): void {
    ...
    this.api.generateSystemAlerts().pipe(
      timeout({ first: 10000 }),
      catchError((error: any) => {
        this.error = this.errorText(error, 'Unable to generate system alerts.');
        return of(null);
      }),
      finalize(() => {
      ...
      if (response) {
        this.message = response?.message || 'System alerts checked.';
        this.load();
      }
    });
  }
